...
export const RoomView = () => {
  ...
  const { questions, isLoading, submitQuestion, upvoteQuestion } = useQuestions(shareLink);
  ...
  const student = session.user as any;
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-300 ${
        ...
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <div>
            ...
            <p className={`mt-1 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
              Welcome, {student.name} from {student.college_name}
            </p>
          </div>
      ...
      <QuestionInput 
        onSubmit={(text) => submitQuestion(text, student.id)} 
        roomId={shareLink}
      />
    </div>
  );
